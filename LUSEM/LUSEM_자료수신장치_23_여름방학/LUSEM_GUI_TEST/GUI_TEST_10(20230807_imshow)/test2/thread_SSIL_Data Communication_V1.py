...
##########################################################################################
        ...
        self.socket_stop_flag = threading.Event()
        self.plot_stop_flag = threading.Event()
...
##########################################################################################
    def func_btn_connect(self):
        ...
        self.thread_socket = threading.Thread(target=self.func_tcpip_run)
        
        while not self.socket_stop_flag.is_set():
            try:
                self.func_tcpip_run()
    ...
    def func_btn_plot_start(self):
        self.thread_plot = threading.Thread(target=self.sci_plot_start)
        ...
        self.verticalLayout.addWidget(self.dynamic_canvas)
        
    def sci_plot_start(self):
        self.timer = self.dynamic_canvas.new_timer(self.timeEdit_interval, [(self.func_sci_plot, (), {})])
        if not self.plot_stop_flag.is_set():
            self.timer.start()
