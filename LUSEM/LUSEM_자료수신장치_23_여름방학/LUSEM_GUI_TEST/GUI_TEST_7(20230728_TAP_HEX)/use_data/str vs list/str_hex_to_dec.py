...
def sci_hex_dec(data):
    ...
    if sci_flag=='40': # compression (x), extension (o), 2 * 512 = 1024 bytes
        ...
        decimal_values = list(int(data[42:42+1024][i:i+2], 16) for i in range(0, len(data[42:42+1024]), 2))
        sci_data.append(decimal_values)
    elif sci_flag=='a0': # compression(o), extension (x), 1* 256 = 256 bytes
        decimal_values = list(int(data[42:42+512][i:i+2], 16) for i in range(0, len(data[42:42+512]), 2))
        sci_data.append(decimal_values)
    else:
        pass # nan list?
