...
incidient_columns = ['position','momentum_direction','theta','phi','Incident_energy']
...
class MyPrimaryGeneratorAction(G4VUserPrimaryGeneratorAction):
    ...
    def GeneratePrimaries(self, anEvent):
        ...
        self.position = self.generator.GetParticlePosition()/cm

        self.momentum_direction = self.generator.GetParticleMomentumDirection()

        self.theta = self.generator.GetParticleMomentumDirection().getTheta()/deg
        self.phi = self.generator.GetParticleMomentumDirection().getPhi()/deg
        
        
        self.initial_energy = self.generator.GetParticleEnergy()/keV
        self.initial_energy = round(self.initial_energy,4)
        ...
        dataframe_incidient.loc[anEvent.GetEventID(), 'Incident_energy'] = self.initial_energy
        dataframe_incidient.loc[anEvent.GetEventID(), 'position'] = self.position
        dataframe_incidient.loc[anEvent.GetEventID(), 'momentum_direction'] = self.momentum_direction
