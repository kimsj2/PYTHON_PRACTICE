...
Dets = ['particle_name', 'config','Incident_energy','Detected_energy']
...
for x in logiclist:
    ...
    dataframe_logiclist[x] = pd.DataFrame(columns=columns, index=index_values)
...
class MyRunAction(G4UserRunAction):
    ...
    def EndOfRunAction(self, aRun):
        dataframe.to_csv('LUSEM_electron_main.csv')
        for x in logiclist:
            if x in rightlogic:
    ...
    def EndOfEventAction(self, anEvent):
        ...
        if self.config == "":
            ...
            for i in range(len(columns)):
                ...
                dataframe_logiclist[self.config].iloc[fit_energy-Emin,i] = self.FTUO[i]
