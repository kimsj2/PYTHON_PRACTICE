...
class MyPrimaryGeneratorAction(G4VUserPrimaryGeneratorAction):
    ...
    def __init__(self):
        ...
        self.particle_energy = self.Emin + G4UniformRand()*((self.Emax*keV-self.Emin*keV))
...
class MyActionInitialization(G4VUserActionInitialization):
    ...
    def Build(self):
        ...
        runAction = MyRunAction(MyPrimaryGeneratorAction())
...
class MyRunAction(G4UserRunAction):
    ...
    def __init__(self, primarygeneratorAction):
        super().__init__()
        # self.Dets = ['particle_name', 'config','Incident_energy','Detected_energy'] # ' BF', 'BT2', 'BT1', 'BU2','BU1', 'BO'
        self.Dets = ['particle_name', 'AF', 'AT2', 'AT1', 'AU2', 'AU1', 'AO','config','Incident_energy','Detected_energy','F','T','U','O']
        self.dataframe = pd.DataFrame(columns=self.Dets)
        self.logiclist = ['F','T','U','O','FT','FU','FO','TU','TO','UO','FTU','FTO','FUO','TUO','FTUO']
        self.rightlogic = ['F','O','FT','UO','FTU','TUO','FTUO']
        self.dataframe_logiclist = {}
        self.columns = ['F','T','U','O']
        self.primarygeneratorAction = primarygeneratorAction
        self.Emax = self.primarygeneratorAction.Emax
        self.Emin = self.primarygeneratorAction.Emin
        
        self.Ebin = 1 # keV
        
        self.particle_energy = self.primarygeneratorAction.particle_energy
        
        for x in self.logiclist:
            index_values = [str(self.Emin + i) for i in range(0, self.Emax - self.Emin + 1, self.Ebin)]
            # data = np.zeros((len(index_values), len(self.columns)))
            self.dataframe_logiclist[x] = pd.DataFrame(columns=self.columns, index=index_values)
            
    def EndOfRunAction(self, aRun):
        self.dataframe.to_csv('LUSEM_electron_main.csv')
        
        for x in self.logiclist:
            if x in self.rightlogic:
                self.dataframe_logiclist[x].to_csv(f'detail_data/right_logic/LUSEM_electron_{x}.csv')
            else:
                self.dataframe_logiclist[x].to_csv(f'detail_data/wrong_logic/LUSEM_electron_{x}.csv')
...
class MyEventAction(G4UserEventAction):
    ...
    def __init__(self,runAction):
        ...
        self.count = [0] * self.detector_num
        self.logic = [0] * 8  # 4(F,FT,FTU,FTUO), 8, 16
        self.Dets = ['AF', 'AT2', 'AT1', 'AU2', 'AU1', 'AO',' BF', 'BT2', 'BT1', 'BU2','BU1', 'BO'] # ' BF', 'BT2', 'BT1', 'BU2','BU1', 'BO'
        self.Det_s = ['particle_name', 'AF', 'AT2', 'AT1', 'AU2', 'AU1', 'AO','config','Incident_energy','Detected_energy','F','T','U','O']
        ...
        self.Emax = self.runAction.Emax
        self.Emin = self.runAction.Emin
        
        self.Ebin = self.runAction.Ebin
        
        self.particle_energy = self.runAction.particle_energy
    ...
    def EndOfEventAction(self, anEvent):
        ...
        for i in range(self.detector_num):
            ...
            for _, edep in evtMap:
                ...
                self.runAction.dataframe.loc[anEvent.GetEventID(), self.Det_s[i + 1]] = edep / keV
        ...
        self.runAction.dataframe.loc[anEvent.GetEventID(), 'Incident_energy'] = self.particle_energy
        self.runAction.dataframe.loc[anEvent.GetEventID(), 'Detected_energy'] = self.detected_energy
        self.runAction.dataframe.loc[anEvent.GetEventID(), 'config'] = self.config
        
        for i in range(len(self.runAction.columns)):
            fit_energy = int((self.FTUO[i]-self.FTUO[i]%1))
            self.runAction.dataframe_logiclist[self.config].iloc[fit_energy-self.Emin,i] = self.FTUO[i]
        
        for i in range(10,14):
            self.runAction.dataframe.loc[anEvent.GetEventID(), self.Det_s[i]] = self.FTUO[i-10]
