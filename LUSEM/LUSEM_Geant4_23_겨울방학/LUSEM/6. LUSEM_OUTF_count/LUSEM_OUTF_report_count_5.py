...
for x in logiclist:
    index_values = [str(Emin + i) for i in range(0, Emax - Emin + 1, Einterval)]
...
class MyRunAction(G4UserRunAction):
    ...
    def BeginOfRunAction(self, aRun):
        ...
        createFolder(f'detail_data({self.start_time_str})/right_logic')
        createFolder(f'detail_data({self.start_time_str})/wrong_logic')
        
    def EndOfRunAction(self, aRun):
        for x in logiclist:
            if x in rightlogic:
                dataframe_logiclist[x].to_csv(f'detail_data({self.start_time_str})/right_logic/LUSEM_{x}.csv')
            else:
                dataframe_logiclist[x].to_csv(f'detail_data({self.start_time_str})/wrong_logic/LUSEM_{x}.csv')
    ...
    def EndOfEventAction(self, anEvent):
        ...
        if self.config != "":
            ...
            for i in range(len(columns)):
                fit_energy = int((self.FTUO[i]-self.FTUO[i]%Einterval))
