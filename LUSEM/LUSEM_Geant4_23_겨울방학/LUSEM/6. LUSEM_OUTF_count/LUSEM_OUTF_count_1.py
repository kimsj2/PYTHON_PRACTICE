...
class MyPrimaryGeneratorAction(G4VUserPrimaryGeneratorAction):
    ...
    def __init__(self):
        ...
        self.particle_energy = random.uniform(100, 10000) * keV
...
class MyRunAction(G4UserRunAction):
    ...
    def __init__(self):
        super().__init__()
        self.Dets = ['particle_name', 'AF', 'AT2', 'AT1', 'AU2', 'AU1', 'AO','errorcount', 'F','FT','FTU','FTUO','O','OU','OUT','OUTF','F (MeV)','T (MeV)','U (MeV)','O (MeV)'] # ' BF', 'BT2', 'BT1', 'BU2','BU1', 'BO'
        self.dataframe = pd.DataFrame(columns=self.Dets)
        
    def EndOfRunAction(self, aRun):
        self.dataframe.to_csv('LUSEM_electron.csv')
    ...
    def EndOfEventAction(self, anEvent):
        ...
        for i in range(self.detector_num):
            ...
            for _, edep in evtMap:
                ...
                self.runAction.dataframe.loc[anEvent.GetEventID(), self.Dets[i + 1]] = edep / MeV
                self.realdata[i] = edep / MeV
                    
        error_count = self.error_count
        if self.realdata[0]!=0:
            if self.realdata[1]!=0:
                if self.realdata[2]!=0:
                    if self.realdata[3] !=0:
                        if self.realdata[4] !=0:
                            if self.realdata[5] !=0:
                                self.logic[3] +=1 # FTUO
                            else:
                                if self.realdata.count(0)!=1:
                                    self.error_count+=1
                                else:
                                    self.logic[2] +=1 # FTU
                        else:
                            if self.realdata.count(0)!=2:
                                self.error_count+=1
                            else:
                                self.logic[2] +=1 # FTU
                    else:
                        if self.realdata.count(0)!=3:
                            self.error_count+=1
                        else:
                            self.logic[1] +=1 # FT
                else:
                    if self.realdata.count(0)!=4:
                        self.error_count+=1
                    else:
                        self.logic[1] +=1 # FT
            else:
                if self.realdata.count(0)!=5:
                    self.error_count+=1
                else:
                    self.logic[0] +=1 # F

        elif self.realdata[5]!=0:
            if self.realdata[4] !=0:
                if self.realdata[3] !=0:
                    if self.realdata[2] !=0:
                        if self.realdata[1] !=0:
            ...
            if self.realdata.count(0)!=6:
                self.error_count+=1

        self.runAction.dataframe.loc[anEvent.GetEventID(), 'errorcount'] = self.error_count
        if self.error_count == error_count:
            for i in range(self.detector_num):
                evtMap = HCE.GetHC(i)
                for _, edep in evtMap:
                    if i in [2, 4]:
                        self.FTUO[i - 1] += edep / MeV
                    elif i in [0, 1]:
                        self.FTUO[i] = edep / MeV
                    elif i == 3:
                        self.FTUO[i - 1] = edep / MeV
                    elif i == 5:
                        self.FTUO[i - 2] = edep / MeV
        
        for i in range(8, 16):
            self.runAction.dataframe.loc[anEvent.GetEventID(), self.Dets[i]] = self.logic[i - 8]
        for i in range(16, 20):
            self.runAction.dataframe.loc[anEvent.GetEventID(), self.Dets[i]] = self.FTUO[i - 16]
