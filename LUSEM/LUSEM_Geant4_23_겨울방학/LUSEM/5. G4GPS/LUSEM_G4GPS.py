...
class LusemPrimaryGeneratorAction(G4VUserPrimaryGeneratorAction):
    ...
    def __init__(self):
        ...
        # UImanager.ApplyCommand("/gps/ang/minphi 0 deg")
        # UImanager.ApplyCommand("/gps/ang/maxphi 360 deg")
        
        UImanager.ApplyCommand("/gps/particle e-")
        UImanager.ApplyCommand("/gps/ene/type Lin")
        UImanager.ApplyCommand("/gps/ene/gradient 0")
        UImanager.ApplyCommand("/gps/ene/intercept 10 keV")
    ...
    def GeneratePrimaries(self, anEvent):
        ...
        self.initial_position = self.generator.GetParticlePosition()/cm  # Get initial position in millimeters
        self.initial_energy = self.generator.GetParticleEnergy()/keV
        self.initial_energy_x = self.generator.GetParticlePosition().getX()/cm
        # # Get initial momentum
        # if self.initial_momentum is None:
        #     self.initial_momentum = self.generator.GetParticleMomentum()/keV  # Get initial momentum in GeV
        
        # Get initial theta and phi
        # if self.initial_theta is None or self.initial_phi is None:
        #     momentum_direction = self.generator.GetParticleMomentumDirection()
        #     self.initial_theta = momentum_direction
        
        print("Initial Position:", self.initial_position)
        print(self.initial_energy_x)
        print("Initial Energy:", round(self.initial_energy,4))
        
        
        
        self.initial_position = None
        self.initial_momentum_direction = None
        self.initial_energy = None
...
class LusemActionInitialization(G4VUserActionInitialization):
    ...
    def Build(self):
        ...
        # self.SetUserAction(LusemStackingAction())
        # self.SetUserAction(LusemTrackingAction())
        # self.SetUserAction(LusemSteppingAction())
...
class LusemEventAction(G4UserEventAction):
    ...
    def EndOfEventAction(self, anEvent):
        ...
        particle = generatorAction.generator.GetParticleDefinition()
