...
file_split_num = 100_000
detected_energy_time = 5

bin_columns = ['Incident_energy','Detected_energy','F','O','FT','UO','FTU','TUO','FTUO'] # 'T','U','FU','FO','TU','TO','FTO','FUO'


detail_folder = f'detail_data_writesplit_{Run_num}'
...
class MyPrimaryGeneratorAction(G4VUserPrimaryGeneratorAction):
    ...
    def GeneratePrimaries(self, anEvent):
        ...
        event_num = anEvent.GetEventID()
        file_num = int(event_num/file_split_num)
        with open(detail_folder +f'/Incident_energy_{file_num}.txt','a') as f:
            f.write(particle_name+ '\t'+ str(self.initial_energy))
            f.write('\n')
...
class MyEventAction(G4UserEventAction):
    ...
    def EndOfEventAction(self, anEvent):
        ...
        event_num = anEvent.GetEventID()
        file_num = int(event_num/detected_energy_time*file_split_num)
        
        with open(detail_folder+f'/Detected_energy_{file_num}.txt','a') as f:
            f.write(self.config+'\t'+str(self.detected_energy))
            f.write('\n')
            
        right_logic = ['F','O','FT','UO','FTU','TUO','FTUO']
        if self.config != "A":
            if (self.detected_energy >= Emin):
                if self.config[1:] in right_logic:
                    fit_energy = int((self.detected_energy-self.detected_energy%Einterval))
                    dataframe_bins.at[fit_energy, self.config[1:]] +=1
