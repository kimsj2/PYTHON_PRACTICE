...
def get_dataset(size):
    ...
    df = pd.DataFrame(index = index, columns = main_columns)
    df['particle_name'] = df['particle_name'].astype('float32')
    df['config'] = df['config'].astype('category')
    df['Incident_energy'] = df['Incident_energy'].astype('float32')
    df['Detected_energy'] = df['Detected_energy'].astype('float32')
    return df
...
dataframe['particle_name'] = dataframe['particle_name'].astype('float32')
dataframe['config'] = dataframe['config'].astype('category')
dataframe['Incident_energy'] = dataframe['Incident_energy'].astype('float32')
dataframe['Detected_energy'] = dataframe['Detected_energy'].astype('float32')
...
class MyActionInitialization(G4VUserActionInitialization):
    ...
    def Build(self):
        ...
        self.SetUserAction(MyPrimaryGeneratorAction(eventAction))
...
class MyRunAction(G4UserRunAction):
    ...
    def BeginOfRunAction(self, aRun):
        ...
        dataframe.iloc[0, 4] = self.start_time_str
    ...
    def EndOfRunAction(self, aRun):
        ...
        dataframe.iloc[0, 5] = end_time_str
        dataframe.iloc[0,6] = elapsed_time
        
        dataframe.to_feather(self.filename+'/LUSEM_main.feather')
        dataframe_bins.to_feather(self.filename +'/LUSEM_bin.feather')
...
class MyEventAction(G4UserEventAction):
    ...
    def EndOfEventAction(self, anEvent):
        ...
        size =10_000
        if self.anEvent_num==0:
            dataframe_split = get_dataset(size)
        if self.anEvent_num >
        index_num = int(self.anEvent_num/size)
        dataframe_split.iloc[index_num, 3] = self.detected_energy
