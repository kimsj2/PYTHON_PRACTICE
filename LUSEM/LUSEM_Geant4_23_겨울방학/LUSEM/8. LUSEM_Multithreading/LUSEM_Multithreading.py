...
main_columns = ['particle_name', 'config','Incident_energy','Detected_energy','AF', 'AT2', 'AT1', 'AU2', 'AU1', 'AO','start_time','end_time','total_time']
...
class MyPrimaryGeneratorAction(G4VUserPrimaryGeneratorAction):
    ...
    def GeneratePrimaries(self, anEvent):
        ...
        particle_energy = Emin*keV + G4UniformRand()*((Emax*keV-Emin*keV))
        self.fParticleGun.SetParticleEnergy(particle_energy)
        particle_energy = particle_energy/keV
        particle_energy = round(particle_energy,4)
        
        dataframe.loc[anEvent.GetEventID(), 'particle_name'] = self.fParticleGun.GetParticleDefinition().GetParticleName()
        
        dataframe.loc[anEvent.GetEventID(), 'Incident_energy'] = particle_energy
        fit_energy = int((particle_energy-particle_energy%Einterval))
...
class MyActionInitialization(G4VUserActionInitialization):
    ...
    def Build(self):
        ...
        # self.SetUserAction(MySteppingAction(MyEventAction()))

class MyRunManager(G4MTRunManager):

    def __init__(self):
        super().__init__()
        self.detector_construction = MyDetectorConstruction()
        self.SetUserInitialization(self.detector_construction)
        self.physics_list = QGSP_BERT()
        self.SetUserInitialization(self.physics_list)
        self.SetUserInitialization(MyActionInitialization())
...
class MyRunAction(G4UserRunAction):
    ...
    def BeginOfRunAction(self, aRun):
        ...
        self.filename = f'detail_data_STEP({self.start_time_str})'
    ...
    def EndOfEventAction(self, anEvent):
        ...
        for i in range(self.detector_num):
            ...
            for _, edep in evtMap:
                dataframe.loc[self.anEvent_num, self.Dets[i]] = edep/keV
                if self.Dets[i][1] not in self.config:
                    self.config += self.Dets[i][1]
                
                self.detected_energy += edep/keV
        
        self.detected_energy = round(self.detected_energy,4)
        
        dataframe.loc[self.anEvent_num, 'Detected_energy'] = self.detected_energy
        
        right_logic = ['F','O','FT','UO','FTU','TUO','FTUO']
        if self.config != "":
            if (self.detected_energy >= Emin):
                if self.config in right_logic:
                    dataframe.loc[self.anEvent_num, 'config'] = self.config
...
def runBatch(num):
    ...
    runManager.SetNumberOfThreads(4)  # 원하는 스레드 수로 설정
    runManager.SetNumberOfEventsToProcess(num)  # 처리할 이벤트 수 설정
    runManager.StartEventLoop(num)
