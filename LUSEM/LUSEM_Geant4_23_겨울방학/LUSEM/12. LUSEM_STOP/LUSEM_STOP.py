...
incidient_columns = ['x','y','z','Px', 'Py','Pz','theta','phi']
...
dataframe_incidient = pd.DataFrame(columns=incidient_columns)
...
class MyPrimaryGeneratorAction(G4VUserPrimaryGeneratorAction):
    ...
    def GeneratePrimaries(self, anEvent):
        ...
        self.x = round((self.generator.GetParticlePosition().getX()/cm),round_num)
        self.y = round((self.generator.GetParticlePosition().getY()/cm),round_num)
        self.z = round((self.generator.GetParticlePosition().getZ()/cm),round_num)
        
        self.Px = round((self.generator.GetParticleMomentumDirection().getX()/cm),round_num)
        self.Py = round((self.generator.GetParticleMomentumDirection().getY()/cm),round_num)
        self.Pz = round((self.generator.GetParticleMomentumDirection().getZ()/cm),round_num)

        self.theta = round((self.generator.GetParticleMomentumDirection().getTheta())/deg,round_num)
        self.phi = round((self.generator.GetParticleMomentumDirection().getPhi())/deg,round_num)
        
        
        self.initial_energy = self.generator.GetParticleEnergy()/keV
        self.initial_energy = round(self.initial_energy,round_num)
        
        dataframe.loc[anEvent.GetEventID(), 'particle_name'] = self.generator.GetParticleDefinition().GetParticleName()
        dataframe.loc[anEvent.GetEventID(), 'Incident_energy'] = self.initial_energy
        
        dataframe_incidient.loc[anEvent.GetEventID(), 'x'] = self.x
        dataframe_incidient.loc[anEvent.GetEventID(), 'y'] = self.y
        dataframe_incidient.loc[anEvent.GetEventID(), 'z'] = self.z
        
        dataframe_incidient.loc[anEvent.GetEventID(), 'Px'] = self.Px
        dataframe_incidient.loc[anEvent.GetEventID(), 'Py'] = self.Py
        dataframe_incidient.loc[anEvent.GetEventID(), 'Pz'] = self.Pz
        
        dataframe_incidient.loc[anEvent.GetEventID(), 'theta'] = self.theta
        dataframe_incidient.loc[anEvent.GetEventID(), 'phi'] = self.phi
...
class MyRunAction(G4UserRunAction):
    ...
    def BeginOfRunAction(self, aRun):
        ...
        self.filename = f'detail_data_gps_{Run_num}_({self.start_time_str})'
    ...
    def EndOfRunAction(self, aRun):
        ...
        dataframe_incidient.to_csv(self.filename +'/LUSEM_incident.csv')
...
class MySteppingAction(G4UserSteppingAction):
    ...
    def UserSteppingAction(self, aStep):
        # Get the kinetic energy of the particle in MeV
        kinetic_energy = aStep.GetTrack().GetKineticEnergy()/keV
        
        # Check if the kinetic energy is less than 1 keV
        if kinetic_energy < 1:
            # Optional: Visualize the particle's trajectory using the vis manager
            vis_manager = G4VisManager.GetConcreteInstance()
            if vis_manager:
                vis_manager.GetCurrentScene().KeepOnlyThis()
                vis_manager.SetCurrentSceneToDraw()
                vis_manager.DrawAll()
