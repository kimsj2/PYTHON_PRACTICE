...
class RutherfordSteppingAction(G4UserSteppingAction):
    ...
    def UserSteppingAction(self, astep):
        ...
        # data.append({'x': position.x, 'y': position.y, 'z': position.z})
        track = astep.GetTrack()
        
        if track.GetVolume().GetName().startswith("FluorescentScreen"):
            # Get the position at the end of the step
            position = astep.GetPostStepPoint().GetPosition()
            eventID = G4RunManager.GetRunManager().GetCurrentEvent().GetEventID()
            VolumeID = astep.GetPreStepPoint().GetTouchable().GetVolume().GetCopyNo()
            AnalysisManager = G4AnalysisManager.Instance()
